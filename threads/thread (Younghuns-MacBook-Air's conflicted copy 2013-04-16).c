...
unsigned int list_insert_start, list_insert_overhead, list_insert_count;
#endif
/* end of Project 3                             */
...
void thread_sleep (int64_t tick)
{
  ...
  old_level = intr_disable ();
#ifdef DEBUG
  unsigned int list_insert_overhead;
  if (trace_scheduler) list_insert_start = rdtsc();
#endif   

  t->wakeup_time = (start + tick);
  list_insert_ordered(&wait_list, &t->elem, less_wakeup_time, NULL);
#ifdef DEBUG
  if (trace_scheduler) list_insert_overhead = list_insert_overhead + rdtsc() - list_insert_start;
  list_insert_count++;
#endif     
  thread_block();
  intr_set_level(old_level);
